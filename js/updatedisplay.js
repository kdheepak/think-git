...
Reveal.addEventListener( 'title-event', function() {
    ...
    var h = 50*1.5*JSONData0_title.nodes.length;

    setTimeout(displayData, 500, JSONData0_title, vis, h);

    setTimeout(displayData, 1500, JSONData1_title, vis, h);

    setTimeout(displayData, 2500, JSONData2_title, vis, h);
} );
...
Reveal.addEventListener( 'fragmentshown', function( event ) {
    ...
    if (classAttribute.indexOf("demo1-git-add") > -1) {
        setTimeout(displayData, 0, JSONData0_demo1, vis, h);
        // do stuff
    }
    if (classAttribute.indexOf("demo1-git-commit") > -1) {
        setTimeout(displayData, 0, JSONData1_demo1, vis, h);
        // do stuff
    }
    if (classAttribute.indexOf("demo1-git-push") > -1) {
        setTimeout(displayData, 0, JSONData2_demo1, vis, h);
        // do stuff
    }
...
Reveal.addEventListener( 'fragmenthidden', function( event ) {
    ...
    var classAttribute = event.fragment.getAttribute("class")
    if (classAttribute.indexOf("demo1-git-add") > -1) {
        setTimeout(displayData, 0, JSONData0, vis, h);
        // do stuff
    }
    if (classAttribute.indexOf("demo1-git-commit") > -1) {
        setTimeout(displayData, 0, JSONData0_demo1, vis, h);
...
function displayData(data, vis, h) {
render(data, vis, h);
}
